...
def _historical_base_rates(
    ...
    """Base-rate disponible en tiempo real.

    Sólo usa observaciones cuyo horizonte completo ya había terminado en la
    fecha de predicción. No usa la frecuencia futura del periodo de test.
    """
    rows = []
...
def walk_forward_backtest(
    ...
    Además guarda un benchmark "rolling base-rate" calculado únicamente con
    resultados que ya eran observables en esa fecha.
    """
...
def calibration_summary(bt: pd.DataFrame) -> pd.DataFrame:
    """Métricas de calibración y Brier frente a un base-rate sólo-pasado."""
    ...
    for h, g in bt.groupby("horizon"):
        for label, pcol, bcol, ycol in events:
            ...
            skill = (
                np.nan
                if not np.isfinite(base_brier) or base_brier <= 0
                else float(1 - brier / base_brier)
            )
            rows.append(
                {
                    "Horizonte": f"{h}D",
...
def directional_summary(bt: pd.DataFrame, upper: float = 0.60, lower: float = 0.40) -> pd.DataFrame:
    """Evalúa señales direccionales donde el modelo se moja (>=60% o <=40%)."""
    if bt is None or bt.empty:
        return pd.DataFrame()
...
def calibration_bins(bt: pd.DataFrame, bins=(0.0, 0.4, 0.5, 0.6, 1.000001)) -> pd.DataFrame:
    """Agrupa P(cerrar arriba) para ver si 60% significa realmente ~60%."""
    if bt is None or bt.empty:
        return pd.DataFrame()
...
def human_verdict(cal: pd.DataFrame, direction: pd.DataFrame) -> str:
    ...
    hits = (
        direction["Acierto"].replace([np.inf, -np.inf], np.nan).dropna()
    ...
    if np.isfinite(mean_skill) and mean_skill > 0.05 and np.isfinite(mean_hit) and mean_hit >= 0.55:
        return "🟢 PROMETEDOR: fuera de muestra el motor mejora al benchmark rolling y las señales fuertes muestran ventaja. Aún hay que confirmar estabilidad por periodos y añadir costes."
    ...
    return "🔴 NO DEMOSTRADO: con esta muestra el motor no mejora de forma consistente al benchmark o falla cuando se moja. Hay que modificarlo antes de fiarnos de sus probabilidades."
